#!/usr/bin/env python3
"""
Tiny Pose Estimation Validation

Validates the tiny pose model on COCO validation set.
Uses the same 128x128 input format as training.

Usage:
    python val_pose_tiny.py --checkpoint pose_tiny_train2/best.pth
    python val_pose_tiny.py --checkpoint pose_tiny_train2/best.pth --visualize
    python val_pose_tiny.py --checkpoint pose_tiny_train2/best.pth --subset 500
"""
...
class TinyStudentModel(nn.Module):
    """Tiny student model architecture (same as train_pose_tiny.py)"""
    def __init__(self):
        super().__init__()
...
def infer(net, img, input_size=128, stride=8):
    ...
    Args:
        ...
        input_size: Input size (128 for tiny model)
        stride: Output stride (8 for tiny model)
...
def visualize_keypoints(img, coco_keypoints, window_name='Tiny Pose Estimation'):
...
def main():
    parser = argparse.ArgumentParser(description='Tiny Pose Estimation Validation')
    parser.add_argument('--checkpoint', type=str, required=True,
                       help='Path to model checkpoint (e.g., pose_tiny_train2/best.pth)')
    ...
    log('=== TINY POSE VALIDATION ===', log_file)
