#!/usr/bin/env python3
"""
Compact (≈2 MB FP32) Pose Training with Distillation.

- Mirrors the flow of train_pose_flexible.py but swaps in a much narrower
  student network (~0.36M params, ~1.5 MB FP32) to target a ~2 MB final
  checkpoint.
- Runs on GPU or CPU, uses the same 128x128 inputs and COCO-style outputs
  (19 heatmaps, 38 PAFs), and distills from the existing 8 MB teacher.
"""
...
DEFAULT_MODEL_DIR = '/content/drive/MyDrive/MLonMCU/Models/MAX78000/IP_2MB'
...
class StudentModel2MB(nn.Module):
    ...
    Structure mirrors the original student but with fewer channels.
    """
...
def main():
    ...
    log("=== FLEXIBLE POSE TRAINING (2 MB STUDENT) ===", log_file)
    ...
    student = StudentModel2MB().to(device)
