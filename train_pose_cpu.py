#!/usr/bin/env python3
"""
Fast Pose Estimation Training with Batch-Level Checkpointing

Saves every N batches (not epochs) so you never lose progress!
Uses smaller subset for faster iteration.

Usage:
    python train_pose_cpu.py --subset 5000 --save-every 100
"""
        ...
        for ki, kp in enumerate(label['keypoints'][:18]):
            if len(kp) >= 3 and kp[2] <= 1:
                ...
                if 0 <= x < out_s and 0 <= y < out_s:
                    # Use proper Gaussian with sigma
                    n_sigma = 4
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str, default=None, help='Path to checkpoint to start from. If not provided, will check for latest.pth or best.pth in output directory')
    parser.add_argument('--output', type=str, default='pose_fast_checkpoints')
    ...
    log(f"=== FAST POSE TRAINING ===", log_file)
    ...
    ckpt = torch.load(os.path.join(AI8X_REPO, 'models/checkpoint_iter_370000.pth'), map_location='cpu')
    load_state(teacher_net, ckpt)
    teacher = TeacherWrapper(teacher_net)
    teacher.eval()
    log(f"Teacher loaded", log_file)
    ...
    while batch_num < args.total_batches:
        ...
        for imgs, heatmaps, pafs in loader:
            ...
            # Create masks (all ones for now, but proper implementation would use actual masks)
            ...
            # Teacher outputs are already interpolated to match student size in TeacherWrapper
            # Use same normalization as student loss for consistency
