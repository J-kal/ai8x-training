#!/usr/bin/env python3
"""
~4 MB FP32 pose student (≈1.0M params):
- Wider backbone and CPM/head than the 2 MB variants.
- CPM/head pre-logits at 144 channels; outputs stay 19/38.
- Same 128x128 input; FP32 by default (AMP opt-in).
"""
...
# Repo paths (Colab/Drive friendly defaults)
POSE_REPO = '/content/lightweight-human-pose-estimation.pytorch'
AI8X_REPO = '/content/ai8x-training'
# Default Drive location for checkpoints
DEFAULT_MODEL_DIR = '/content/drive/MyDrive/MLonMCU/Models/MAX78000/IP_4MB'
...
class StudentModel4MB(nn.Module):
    """
    Wider student targeting ~4 MB FP32 (~1.0M params).
    """
    ...
    def __init__(self):
        ...
        self.conv7 = ai8x.FusedConv2dBNReLU(96, 144, 3, stride=1, padding=1, bias=True)
        self.conv8 = ai8x.FusedConv2dBNReLU(144, 144, 3, stride=1, padding=1, bias=True)
        # CPM
        self.cpm1 = ai8x.FusedConv2dBNReLU(144, 144, 1, padding=0, bias=True)
        self.cpm2 = ai8x.FusedConv2dBNReLU(144, 144, 3, padding=1, bias=True)
        self.cpm3 = ai8x.FusedConv2dBNReLU(144, 144, 3, padding=1, bias=True)
        # Heads
        self.heat_conv = ai8x.FusedConv2dBNReLU(144, 144, 1, padding=0, bias=True)
        self.heat_out = ai8x.Conv2d(144, 19, 1, padding=0, bias=True, wide=True)
        self.paf_conv = ai8x.FusedConv2dBNReLU(144, 144, 1, padding=0, bias=True)
        self.paf_out = ai8x.Conv2d(144, 38, 1, padding=0, bias=True, wide=True)
...
def main():
    ...
    parser.add_argument('--amp', dest='amp', action='store_true', default=False,
                        help='Enable mixed precision on CUDA (default: off, FP32 checkpoints)')
    ...
    log("=== FLEXIBLE POSE TRAINING (4 MB STUDENT) ===", log_file)
    ...
    log(f"Mixed precision: {'ON' if enable_amp else 'OFF (FP32)'}", log_file)
    if checkpoint_path:
    ...
    student = StudentModel4MB().to(device)
    param_count = sum(p.numel() for p in student.parameters())
    param_mb = param_count * 4 / (1024 * 1024)
    log(f"Student params: {param_count:,} (~{param_mb:.2f} MB FP32)", log_file)
